...
- **Stress and pitch accent in real words (mostly done).** 32 lexicons carry stress marks, Japanese carries
  pitch accent for ~91% of its polysyllabic words (see `LEXICON_AUDIT.md`). Still open: (a) Arabic, Georgian,
  Hawaiian (no derivable rule; each needs per-word data or a language-specific weight rule; Basque and Hindi
