...
**Morphology (paradigms, affix positions and morphophonology done, passes 22-25):** derivation and
compounding (L); reduplication as grammar (M); root-and-pattern beyond citation shapes (L). Still missing from the
work so far (S-M): harmony is one feature (backness, height or rounding) over vowel pairs the inventory has, with no
neutral-vowel transparency rules beyond "skip vowels outside a pair", no disharmonic loanwords, and it does not reach
compound words or the lexicon's own citation suffixes (`WordClass` harmony is a separate mechanism); the hiatus rule is
elision or one glide (no consonant assimilation, no epenthesis before consonant clusters, no stress-driven vowel
reduction); mutation is lenition of the initial consonant in one or two cells (no nasal or aspirate mutation, and
no mutation triggered by a preceding *word*, as in Welsh); stem changes are one vowel or the final consonant in one
trigger cell; class assignment is by gender or a hash of the gloss; an infix goes after the first consonant or before the
last vowel only. Decoding an unknown token in a language with infixed verbs can still take a few seconds. The plural,
