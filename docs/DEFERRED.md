...
**Morphology (paradigms and affix positions done, passes 22-24):** morphophonology at affix boundaries
(vowel harmony, mutation) (L); derivation and compounding (L); reduplication as grammar (M); root-and-pattern
beyond citation shapes (L). Still missing from the paradigm and position work (S-M): a stem change is one vowel or
the final consonant in one trigger cell (no ablaut series, no alternation across a whole paradigm); class
assignment is by gender or a hash of the gloss; the position is chosen per inflectional field, not per label (no
Bantu-style tense prefix with a suffixed agreement in the same field), only nine noun/verb fields move (adjective
degree and agreement, evidentials, negation, class markers and possessor persons stay as they are), an infix goes
after the first consonant or before the last vowel only (no infixes at syllable or morpheme boundaries), a
circumfix is a fresh prefix around the old suffix (not a genuinely discontinuous morpheme with two parts drawn
together), and prefixes never trigger sandhi with the stem. Decoding an unknown token in a language with infixed
verbs can still take a few seconds. The plural, imperative and question-particle forms do not yet evolve with
`sound_change` (S).
