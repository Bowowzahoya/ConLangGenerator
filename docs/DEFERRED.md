...
**Morphology (paradigms, affix positions, morphophonology, derivation and compounding done, passes 22-26):**
reduplication as grammar (M); root-and-pattern beyond citation shapes (L). Still missing from the derivation and
compounding work (S-M): five derivational rules only (agent, abstract, negative, diminutive, adjectival; no
verbalizers, instruments, places or collectives, no derivation across a rule chain beyond what the base already is); the
English side is a closed suffix analyser that only fires when the base word is already in the language's lexicon
and the planner's part of speech matches the rule's result; a derived word is an ordinary lexicon entry (nothing
records that `teacher` is *productively* `teach` + agent, so evolution changes the two independently after
the fact); compounds are noun + noun only, from a solid or hyphenated English word whose two halves are
both known nouns (no verb + noun, no compound made from a phrase the planner splits into two slots, no semantic
relations between the parts, no clipping or stem-alternation at the join); harmony does not apply across a
compound. Also still open from earlier morphology: a stem change is one vowel or the final consonant in one trigger
cell; class assignment is by gender or a hash of the gloss; an infix goes after the first consonant or before the last
vowel only. Decoding an unknown token in a language with infixed verbs can still take a few seconds. The plural,
